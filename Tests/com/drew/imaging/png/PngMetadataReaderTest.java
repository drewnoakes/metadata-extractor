...
public class PngMetadataReaderTest
{
    ...
    public void testGimpGreyscaleWithManyChunks() throws Exception
    {
        ...
        try {
            ...
            assertEquals(PngChunkType.tIME, dirs[3].getPngChunkType());
            assertEquals("Tue Jan 01 04:08:30 GMT 2013", dirs[3].getDate(PngDirectory.TAG_LAST_MODIFICATION_TIME).toString());

            assertEquals(PngChunkType.iTXt, dirs[4].getPngChunkType());
            @SuppressWarnings("unchecked")
            List<KeyValuePair> pairs = (List<KeyValuePair>)dirs[4].getObject(PngDirectory.TAG_TEXTUAL_DATA);
            assertNotNull(pairs);
