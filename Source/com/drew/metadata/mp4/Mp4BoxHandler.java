...
public class Mp4BoxHandler extends Mp4Handler<Mp4Directory> {
	...
	public boolean shouldAcceptBox(@NotNull String type) {
		return type.equals(Mp4BoxTypes.BOX_FILE_TYPE) || type.equals(Mp4BoxTypes.BOX_MOVIE_HEADER)
				|| type.equals(Mp4BoxTypes.BOX_HANDLER) || type.equals(Mp4BoxTypes.BOX_MEDIA_HEADER)
				|| type.equals(Mp4BoxTypes.BOX_TRACK_HEADER) || type.equals(Mp4BoxTypes.BOX_USER_DATA)
				|| type.equals(Mp4BoxTypes.BOX_USER_DEFINED);
	}

	@Override
	public boolean shouldAcceptContainer(@NotNull String type) {
		return type.equals(Mp4ContainerTypes.BOX_TRACK) || type.equals(Mp4ContainerTypes.BOX_METADATA)
				|| type.equals(Mp4ContainerTypes.BOX_MOVIE) || type.equals(Mp4ContainerTypes.BOX_MEDIA);
	}

	@Override
	public Mp4Handler<?> processBox(@NotNull String type, @Nullable byte[] payload, long boxSize, Mp4Context context)
			throws IOException {
		if (payload != null) {
	...
	private void processFileType(@NotNull SequentialReader reader, long boxSize) throws IOException {
		...
		directory.setStringArray(Mp4Directory.TAG_COMPATIBLE_BRANDS,
	...
	private void processMovieHeader(@NotNull SequentialReader reader) throws IOException {
		...
		int[] matrix = new int[] { reader.getInt32(), reader.getInt32(), reader.getInt32(), reader.getInt32(),
				reader.getInt32(), reader.getInt32(), reader.getInt32(), reader.getInt32(), reader.getInt32() };
	...
	private void processMediaHeader(@NotNull SequentialReader reader, Mp4Context context) throws IOException {
		...
		context.language = new String(new char[] { (char) (((languageBits & 0x7C00) >> 10) + 0x60),
				(char) (((languageBits & 0x03E0) >> 5) + 0x60), (char) ((languageBits & 0x001F) + 0x60) });
	}
