...
//                directory.setDouble(QtDirectory.TAG_SOUND_BALANCE, integerPortion + fractionPortion);
    ...
    private void processSoundSampleDescription(@NotNull QtDirectory directory, SequentialByteArrayReader reader, String dataFormat) throws IOException {
        ...
        switch (version) {
            ...
            case (1):
                ...
                directory.setString(QtDirectory.TAG_AUDIO_FORMAT, QtSampleDescriptionDictionary._dictionary.get(dataFormat));
    ...
    private void processVideoSampleDescription(@NotNull QtDirectory directory, SequentialByteArrayReader reader, String dataFormat) throws IOException {
        ...
        directory.setString(QtDirectory.TAG_COMPRESSION_TYPE, QtSampleDescriptionDictionary._dictionary.get(dataFormat));
    ...
    private void processFileType(@NotNull byte[] payload, SequentialByteArrayReader reader) throws IOException, ImageProcessingException {
        reader.skip(8);
        String majorBrand = new String(reader.getBytes(4));
...
//        }
    ...
    private void processMovieHeader(@NotNull QtDirectory directory, SequentialByteArrayReader reader) throws IOException {
        reader.skip(4);
        long creationTime = reader.getInt32();
        long modificationTime = reader.getInt32();
        ...
        int previewTime = reader.getInt32();
        int previewDuration = reader.getInt32();
        int posterTime = reader.getInt32();
        int selectionTime = reader.getInt32();
        int selectionDuration = reader.getInt32();
        int currentTime = reader.getInt32();
        int nextTrackId = reader.getInt32();


        directory.setLong(QtDirectory.TAG_CREATION_TIME, creationTime);
        directory.setLong(QtDirectory.TAG_MODIFICATION_TIME, modificationTime);
        ...
        directory.setInt(QtDirectory.TAG_PREVIEW_TIME, previewTime);
        directory.setInt(QtDirectory.TAG_PREVIEW_DURATION, previewDuration);
        directory.setInt(QtDirectory.TAG_POSTER_TIME, posterTime);
        directory.setInt(QtDirectory.TAG_SELECTION_TIME, selectionTime);
        directory.setInt(QtDirectory.TAG_SELECTION_DURATION, selectionDuration);
        directory.setInt(QtDirectory.TAG_CURRENT_TIME, currentTime);
        directory.setInt(QtDirectory.TAG_NEXT_TRACK_ID, nextTrackId);
    }
}
